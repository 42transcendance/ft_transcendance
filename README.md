...
| 3 | Major | User interaction | nbacconn, ebenoist | 2 | Covers the social interactions of the app: auth, profile, friends, chat |
...
#### 1. Docker/Podman data-root configuration (first time only)

On the **42 goinfre system**, disk space is limited in user home directories. To avoid running out of space during container builds and execution, you must redirect Podman's data storage to the larger `/goinfre` partition.
...
This command:
- Executes `scripts/docker-goinfre-setup.sh` to configure Podman's `daemon.json` file
- Redirects Docker/Podman data storage from `~/.local/share/docker` to `/goinfre/$USER/docker-data`
- Requires `jq` and `sudo` (or rootless Docker support)
- Restarts your Docker/Podman daemon automatically

After this, all future containers, images, and volumes will be stored in `/goinfre`, freeing up your home directory.

#### 2. Environment and secrets files
...
3. Start the stack with Podman:
	...
	make up-goinfre
	```
	- Use `make up-goinfre` on the 42 goinfre system (ensures correct `DOCKER_DATA_ROOT`)
	- Use `make up` on local systems or if your Podman setup already works normally
